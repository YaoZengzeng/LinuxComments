...
func CreateLibcontainerConfig(opts *CreateOpts) (*configs.Config, error) {
	...
	if !filepath.IsAbs(rootfsPath) {
		// 如果rootfsPath不是绝对路径，默认为"rootfs"
		// 则将其设置为cwd/rootfsPath
		rootfsPath = filepath.Join(cwd, rootfsPath)
	}
	labels := []string{}
	// 将annotation设置为labels
	...
	for _, m := range spec.Mounts {
		// mouts主要对/proc，/dev，/sys等文件系统进行挂载
	...
	if spec.Linux != nil {
		...
		// MaskPaths默认有定义，为/proc/kcore等
		config.MaskPaths = spec.Linux.MaskedPaths
		// ReadonlyPaths默认有定义，为/proc/asound等
	...
	// 创建hook命令，Prestart, PostStart和PostStop
