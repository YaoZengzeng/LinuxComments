...
func (c *linuxContainer) Start(process *Process) error {
	...
	if status == Stopped {
		// 如果容器初始状态为stopped，则先创建exec fifo
		if err := c.createExecFifo(); err != nil {
			return err
		}
	}
	// 如果status为Stopped，则isInit为true
...
// 若初始状态为stopped，则isInit为true
func (c *linuxContainer) start(process *Process, isInit bool) error {
	// 在容器进程运行之前，需要利用父进程和容器进程进行通信，因此这里封装了一个parentProcess的概念
...
func (c *linuxContainer) newParentProcess(p *Process, doInit bool) (parentProcess, error) {
	// parentPipe和childPipe就是父进程和创建出来的容器init进程通信的管道，这个管道用于init容器进程
	// 启动之后做一些配置工作，非常重要
	...
	// 如果容器的状态不为stop，则返回c.newSetnsProcess()
	if !doInit {
...
func (c *linuxContainer) newInitProcess(p *Process, cmd *exec.Cmd, parentPipe, childPipe *os.File) (*initProcess, error) {
	...
	return &initProcess{
		// cmd是init程序，即启动的容器子进程是runc init
		cmd:             cmd,
		childPipe:       childPipe,
		...
		container:       c,
		// process会保存容器init进程初始化需要的数据
		process:         p,
		// bootstrapData中保存了容器init初始化需要的数据，是一个io.Reader
		bootstrapData:   data,
		// 是否共享pidns
...
// as a io.Reader.
// bootstrapData将必须的数据封装成network binary format作为io.Reader
