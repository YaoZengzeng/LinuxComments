...
// finalizeNamespace丢弃所有的caps，并且设置恰当的user和working dir
// 并且在namespace执行命令之前关闭任何泄露的文件描述符
func finalizeNamespace(config *initConfig) error {
	// Ensure that all unwanted fds we may have accidentally
	// inherited are marked close-on-exec so they stay out of the
	// container
	// 确保所有我们不小心继承的文件描述符都被设置为close-on-exec，从而让它们能置于容器之外
	...
	// 设置的container capabilities
	w, err := newContainerCapList(capabilities)
	if err != nil {
		return err
	}
	// drop capabilities in bounding set before changing user
	// 在改变user之前丢弃所有在bounding set中的capabilities
...
// syncParentReady给指定的pipe发送一个JSON payload，用于说明init已经准备Exec子进程了，
// 接着它会等待parent告诉它所有准备就绪，可以exec了
