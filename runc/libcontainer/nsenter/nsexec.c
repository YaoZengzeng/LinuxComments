 ...
 */
// parent和child交互的netlink message的消息类型
 ...
 */
// 从环境变量中获取init pipe fd，它用于读取bootstrap data以及在我们布置好环境之后
// 告诉父进程我们的pid
...
void nsexec(void)
{
	 ...
	 *          process.
	 */
	// 我们是父进程，我们的工作是创建一个新的子进程（stage1）并且写它的uid_map和gid_map，
	// 之后该子进程又会去创建一个新的进程，它会将它的pid发送给我们，我们再将其发送给bootstrap process
	case JUMP_PARENT: {
			int len;
			 ...
			 */
			// 用于和子进程同步的状态机，只有在子进程和孙子进程都ready的情况下父进程才能推出
			while (!ready) {
				enum sync_t s;
				int ret;
	 ...
	 */
	// 我们是第一个子进程，我们的工作是加入所有在netlink payload中提供的namespaces，
	// 并且unshare所有的requested namespaces，如果我们要求CLONE_NEWUSER，我们会让
	// 我们的parent（即stage0）为我们设置user mappings，接着我们创建一个子进程(即stage 2)
	// 接着我们再将我们子进程的pid传递给我们的父进程
	case JUMP_CHILD: {
			pid_t child;
			enum sync_t s;
	 ...
	 */
	// 这一部分仅仅做清理工作并且返回Go runtime运行init_linux.go
	case JUMP_INIT: {
			/*
