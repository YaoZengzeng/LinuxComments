...
// NetworkEndpoint是三层协议的endpoint需要实现的接口
type NetworkEndpoint interface {
	// MTU is the maximum transmission unit for this endpoint. This is
	// generally calculated as the MTU of the underlying data link endpoint
	// minus the network endpoint max header length.
	// 底层data link endpoint的mtu减去network endpoint的header length
	MTU() uint32
...
// the data link layer.
// NetworkDispatcher包含在链路层处理完之后，将数据发往正确的network endpoint的方法，
...
// LinkAddressResolver是NetworkProtocol的扩展，用于那些能够解析链路层地址的网络层协议
...
var (
	// 每个四层协议包的init函数都会在transportProtocols中进行注册
	transportProtocols = make(map[string]TransportProtocolFactory)
	// 与transportProtocols同理
	networkProtocols   = make(map[string]NetworkProtocolFactory)

	linkEPMu           sync.RWMutex
	// 用于表示下一个LinkEndpoint使用的ID
