...
func New(network []string, transport []string) *Stack {
	...
	for _, name := range network {
		...
		// netProto的类型为接口NetworkProtocol
		netProto := netProtoFactory()
		// netProto.Number()就是在报文中实际代表协议的值，对于IPv4就是0x0800
		s.networkProtocols[netProto.Number()] = netProto
		// 将接口NetworkProtocol转换为接口LinkAddressResolver
		// arp也为NetworkProtocol
	...
	for _, name := range transport {
		...
		// transProto的类型为TransportProtocol
