...
// package tcpip用于为tcpip stack的用户提供接口和相关的类型，从而能够通过协议栈发送和接收数据
//
...
// 通过tcp/stack/stack的New()函数创建一个stack结构
// 接着可以通过调用Stack.CreateNIC()创建网卡，通过Stack.AddAddress()添加地址，再通过Stack.SetRouteTable()创建路由表
// 
...
// 通过调用Stack.NewEndpoint()可以创建endpoints，根据不同的协议，我们可以通过endpoints我们可以收发数据
// 连接对端，监听连接以及接收连接
package tcpip
