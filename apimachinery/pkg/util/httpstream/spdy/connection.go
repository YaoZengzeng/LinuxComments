...
// client.
// NewServerConnection创建一个新的SPDY server connection
// 当server从client收到一个新创建的stream时，newStreamHandler就会被调用
...
// newSpdyStream是由spdystream.Connection.Serve内部使用的stream handler
// 它会调用connection的newStreamHandler，给它机会接收或者拒绝stream。如果newStreamHandler报错
// 则stream被拒绝，如果没有报错，则接收并且将其注册到connection中
