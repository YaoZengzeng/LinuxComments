...
// 其中的subnet.Manager是一个interface
// 它要求能够GetNetworkConfig, AcquireLease，RenewLease，WatchLease和WatchLeases
...
func main() {
	...
	if len(opts.iface) == 0 && len(opts.ifaceRegex) == 0 {
		// 当iface和ifaceRegex都未指定时，选取默认的网卡
	...
	// 创建一个network结构并返回，并且调用subnet manager获得一个lease
	...
	// 将FLANNEL_NETWORK,FLANNEL_SUBNET,FLANNEL_MTU,FLANNEL_IPMASQ写入subnetFile中
...
func LookupExtIface(ifname string, ifregex string) (*backend.ExternalInterface, error) {
	...
	if extAddr == nil {
		...
		// 将网卡地址作为external address
		// 在节点间通信时，其他主机可以通过external address访问本节点
		extAddr = ifaceAddr
	}
