...
nbctl_ls_add(struct ctl_context *ctx)
{
    ...
    if (ls_name) {
        ...
        if (!add_duplicate) {
            const struct nbrec_logical_switch *ls;
            // 遍历所有的ls，如果有重复，并且未指定"--may-exist"则报错
            NBREC_LOGICAL_SWITCH_FOR_EACH (ls, ctx->idl) {
...
// 每个结构中的字段依次为：name，name后面最少的arguments数量，最大的arguments数量，
// 对参数的描述，prerequisites函数指针用于获取命令使用的column和table，run函数指针
// 用于具体执行命令，postprocessing函数在输出被提交后执行，options用于标志该命令是否
// 接收选项，最后指定RO或者RW用于区分该命令是否修改数据库
