 ...
 */
// 创建并返回对数据库remote的连接
// 如果monitor_everything_by_default为true，那么默认复制所有远程数据库的所有数据
// 如果monitor_everything_by_default为false，则默认不会复制任何columns或者tables
struct ovsdb_idl *
...
// 当sequence number未发生改变时，client可以继续使用从'idl'中获取的任何数据结构。一旦它发生改变
// client将不再能使用其中的任何数据，因为它们可能已经被修改或用于其他用途了
unsigned int
 ...
 * functions generated by ovsdb-idlc. */
// 通过txn在指定的'class'中加入一个新的row并返回
// 例如在创建新的ls时，class为&nbrec_table_logical_switch
const struct ovsdb_idl_row *
ovsdb_idl_txn_insert(struct ovsdb_idl_txn *txn,
                     const struct ovsdb_idl_table_class *class,
                     const struct uuid *uuid)
{
    // 创建一个ovsdb_idl_row对象，并用class->row_init()函数初始化
