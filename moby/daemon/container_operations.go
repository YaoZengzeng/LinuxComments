...
// updateContainerNetworkSettings更新网络设置
func (daemon *Daemon) updateContainerNetworkSettings(container *container.Container, endpointsConfig map[string]*networktypes.EndpointSettings) {
	...
	// 创建container.NetworkSettings.Network
	if container.NetworkSettings.Networks == nil {
		container.NetworkSettings.Networks = make(map[string]*network.EndpointSettings)
...
func (daemon *Daemon) allocateNetwork(container *container.Container) error {
	...
	// 如果container没有连接到任何network，则直接创建它自己的network sandbox，如果不存在的话
	if len(networks) == 0 {
